...
This repository is in **Application Implementation**. The authorized implementation boundary covers the **Opportunity Work Images vertical slice (Slice 1A)**, the **Site Survey Vertical Slice (Slice 4)**, and the **Official Estimate BOQ & Calculation Vertical Slice (Slice 5A)** documented in `docs/superpowers/plans/2026-09-18-official-estimate-boq-calculation-vertical-slice.md` and `docs/05-engineering/official-estimate-verification.md`. This slice covers multi-image work photo attachment for Open Opportunities, site survey appointment scheduling, surveying stage transitions, survey workspace with areas/measurements/notes, survey revision readiness gates (Mark Ready) with snapshot hashing, automatic progression to estimating stage, official estimate draft creation, BOQ workspace drawer (sections, work items, cost components), real-time pricing rules, and official calculation engine with discount, VAT 7%, and calculation snapshots. Customer lifecycle transitions beyond activation, Site edit/deactivation, Commercial quotation issuance, Project, Procurement, Inventory, Production, and MRP behavior remain deferred without a separately approved implementation task.
