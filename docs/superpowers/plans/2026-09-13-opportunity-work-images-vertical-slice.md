...
| Frontend | Modify `frontend/src/lib/api/api-client.ts`, `frontend/src/features/opportunities/api/opportunity-queries.ts`, `frontend/src/features/opportunities/components/opportunity-detail.tsx`, `frontend/src/messages/th.json`, `en.json`; new reusable `frontend/src/components/forms/MultiImagePicker.tsx` (รองรับเลือกไฟล์และถ่ายภาพด้วยปุ่มสี่เหลี่ยมจัตุรัสคู่สมมาตร), `frontend/src/hooks/useCamera.ts` (WebRTC stream & camera control hook) และ `frontend/src/components/forms/CameraCaptureModal.tsx` (Full-screen camera viewfinder & photo review modal); new `frontend/src/features/opportunities/components/opportunity-work-image-form.tsx` and `opportunity-work-image-gallery.tsx`. |
